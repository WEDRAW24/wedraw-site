    ...
    >
      ...
      <div className="absolute inset-0" style={{ zIndex: 1 }}>
        ...
        {Array.from({ length: GRID_SIZE + 1 }, (_, i) => (
          <div
            key={`v-${i}`}
            className="absolute top-0 bg-marker/30"  // Changed from bg-marker to bg-marker/30
            style={{
              width: '0.5px',
              height: '100%',
              left: `${i * cellSize}px`
        ...
        {Array.from({ length: GRID_SIZE + 1 }, (_, i) => (
          <div
            key={`h-${i}`}
            className="absolute left-0 bg-marker/30"  // Changed from bg-marker to bg-marker/30
            style={{
              height: '0.5px',
              width: '100%',
              top: `${i * cellSize}px`
